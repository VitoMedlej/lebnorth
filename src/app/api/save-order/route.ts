...
export async function POST(req: NextRequest, res: NextApiResponse) {
  try {
    if (req.method === 'POST') {
      ...
      const insertReq = await client.db("LEB").collection("Orders").insertOne(order);
      ...
      if (insertReq.acknowledged) {
        for (const product of order.products) {
          ...
          const existingProduct = await client.db("LEB")
            .collection("Products")
          ...
          const updateStockReq = await client.db("LEB").collection("Products").updateOne(
